.TH "NPX" "1" "June 2017" "npx@2.1.0" "User Commands"
.SH "NAME"
\fBnpx\fR \- execute npm package binaries
.SH SYNOPSIS
.P
\fBnpx [\-\-package|\-p <package>] [\-\-cache <path>] [\-\-userconfig <path>] [\-c <string>] [\-\-shell|\-s <string>] [\-\-no\-install] [\-\-shell\-auto\-fallback [<shell>]] [\-\-ignore\-existing] [\-\-version|\-v] [\-\-] <command>[@version] [command\-arg]\.\.\.\fP
.SH INSTALL
.P
...
.P
An optional \fB@version\fP may be appended to specify the package version required, which defaults to \fBlatest\fP only if \fB<command>\fP is not in the path\. If the command is already present and no explicit version specifier was requested, the existing command will be used\.
.P
If a version specifier is included, or if \fB\-\-package\fP is used, npx will ignore the version of the package in the current path, if it exists\. This can also be forced with the \fB\-\-ignore\-existing\fP flag\.
.RS 0
.IP \(bu 2
...
.IP \(bu 2
\fB\-c <string>\fP \- Execute \fB<string>\fP inside a shell\. For unix, this will be \fB/bin/sh \-c <string>\fP\|\. For Windows, it will be \fBcmd\.exe /d /s /c <string>\fP\|\. Only the first item in \fB<string>\fP will be automatically used as \fB<command>\fP\|\. Any others \fImust\fR use \fB\-p\fP\|\.
.IP \(bu 2
\fB\-\-shell <string>\fP \- The shell to invoke the command with, if any\. Defaults to \fBfalse\fP\|\.
.IP \(bu 2
...
.P
You can configure \fBnpx\fP to run as your default fallback command when you type something in the command line but the command is not found\. This includes installing packages that were not found in the local prefix either\.
.P
